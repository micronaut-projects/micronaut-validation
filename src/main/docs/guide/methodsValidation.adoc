...
NOTE: In Python the class must be annotated with ann:validation.Validated[] (`micronaut.validation.Validated`): the Python compiler only routes calls to the methods of a class through the proxy that performs the validation when the class declares the annotation.
--
