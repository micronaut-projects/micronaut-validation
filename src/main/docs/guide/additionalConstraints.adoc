...
[.lang-python]
--
NOTE: In Python an annotation is a function that returns a decorator; its parameters are the annotation members. Since the annotation references the validator class in `validatedBy`, the validator declares the annotation type argument as a forward reference (`ConstraintValidator["DurationPattern", object]`) with the import guarded by `typing.TYPE_CHECKING` to avoid a circular import.
--
