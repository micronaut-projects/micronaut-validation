...
The above example defines a `Person` class that has two properties (`name` and `age`) that have constraints applied. Note that in Java the annotations can be on the field or the getter, with Kotlin data classes, the annotation should target the field, and in Python the constraints are declared on the attributes of a `dataclass` with `typing.Annotated`.
...
NOTE: This feature is not yet supported in Groovy, Kotlin and Python
