...
final class ReflectiveExecutables {
     ...
     * introspection describes when it is that constructor, another one a reflective introspection knows,
     * else one read from the constructor itself.
     *
    ...
    static <T> BeanConstructor<T> beanConstructor(@Nullable BeanIntrospection<T> introspection, Constructor<T> constructor) {
        Class<?>[] parameterTypes = constructor.getParameterTypes();
